...
int ckvs_client_encrypt_pwd(ckvs_memrecord_t *mr, const char *key, const char *pwd){
    ...
    HMAC(EVP_sha256(), mr->stretched_key.sha, SHA256_DIGEST_LENGTH, AUTH_MESSAGE, strlen(AUTH_MESSAGE), &mr->auth_key.sha, &size);
...
int ckvs_client_compute_masterkey(struct ckvs_memrecord *mr, const struct ckvs_sha *c2){
    ...
    unsigned int size = 0;
    HMAC(EVP_sha256(), mr->c1.sha, SHA256_DIGEST_LENGTH, c2->sha, SHA256_DIGEST_LENGTH, mr->master_key.sha, &size);
    if(size != SHA256_DIGEST_LENGTH){ return ERR_INVALID_COMMAND; }
    return ERR_NONE;
}